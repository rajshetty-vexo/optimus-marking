...
import { labellingData, thermalInkjetData } from "../data/labellingData";
...
const Products = () => {
  ...
  const displayedProducts = [...labellingData.slice(0, 4), ...thermalInkjetData.slice(0, 4)];
  ...
  {cat.logo && (
    ...
    />
                      ...
                      <div className={`absolute inset-0 w-full flex justify-center items-center transition-all duration-500 ease-in-out ${isHovered ? "opacity-100 translate-y-0 z-10" : "opacity-0 translate-y-4 pointer-events-none"}`}>
                        <Link to={`/labelling/${cat.id}`} className="w-full max-w-[135px] text-center text-white text-[10px] font-bold uppercase tracking-widest py-2.5 rounded-sm block transition-all duration-300 shadow-sm bg-[#F97316]">
                          Discover More
                        </Link>
                      </div>
                    </div>
